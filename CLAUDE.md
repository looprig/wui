...
- `json-schema-to-ts` — **type-only**: `packages/protocol` imports `FromSchema` and `JSONSchema` with `import type`, so nothing survives compilation and nothing reaches the browser. It is nevertheless declared under `dependencies` in `packages/protocol/package.json`, not `devDependencies`. That is arguably wrong — a type-only package under `dependencies` makes every consumer install it — but it is deliberately **left in place**: the emitted `packages/protocol/dist/types.d.ts` keeps the `import type { FromSchema } from "json-schema-to-ts"` line verbatim, so anything type-checking against the built package needs it resolvable. Today that is only the workspace (`@looprig/protocol` is `private` and linked, never published), so the move would be safe here and unsafe the moment it is not. Move it only as a considered change, not as tidying.
  ...
  **This justification is now permanent, not provisional.** It was written as "safe here and unsafe the moment it is not" because publishing the package was a scheduled step — runbook 06 task U6.1 step 4, gated on human gate H2. The owner **deferred/deprecated the whole Client lane (`C1.1`–`C1.3`) on 2026-09-13**, and H2 existed only to serve it, so there is no longer a plan under which this package is published. The package stays `private`, the type-only-under-`dependencies` placement stays, and the conditional in the paragraph above should be read as settled rather than pending. `packages/protocol/test/surface.test.ts`'s "declares the private candidate's complete publish boundary" pins `private: true` and `publishConfig.access: "restricted"`, so reversing this needs a test edit and a new owner decision, not a `npm publish`.
...
Standalone verification is `GOWORK=off go test ./...`. Both it and `make check` must pass with **no Node toolchain installed**, which is why two files under `dist/` are committed unconditionally (see `.gitignore`): `index.html`, so `//go:embed all:dist` compiles without a build, and `looprig-bundle.json`, the marker `BundleProtocolVersion` reads — a build with no marker is a build that cannot answer what its bundle speaks. The marker is written by `vite.config.ts`'s `bundleManifestPlugin` on every build, into whichever `--outDir` that build resolved, and it says `release: true` only when `WUI_BUNDLE_RELEASE=1` — which `make release-dist` sets and nothing else does. `node app/scripts/write-bundle-manifest.mjs` is the same writer's CLI, for regenerating the committed marker without a full build. Use `all:dist`, never a bare `dist` — without the prefix, entries whose names begin with `_` or `.` are silently skipped.

**The committed bundle is a release artefact, and stays one.** Task U6.0-shipdist ran `make release-dist` and committed its output, so `dist/looprig-bundle.json` says `release: true` in the tree at rest — not only during a release run. The module zip is source-only, so this is the artefact every consumer `//go:embed`s; there is no later build step that could make it one. `bundle_test.go` pins `Release` by literal alongside the other three marker fields, which means an ordinary `npm run build` — the plugin writes `release: false` without `WUI_BUNDLE_RELEASE=1` — turns `make check` red rather than quietly downgrading what ships. Run `make dist-reset` to restore the committed bundle after a development build; use `make release-dist` to move it. The older rule that the marker must EQUAL `WUI_BUNDLE_RELEASE` was removed with this task: it was written when the committed tree was expected to be a development build, and on a correct release tree it failed every developer and CI run. What survives is one-directional — a release run must produce a release marker.
